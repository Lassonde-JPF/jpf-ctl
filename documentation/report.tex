...
We denote the set of (actual) paths defined in Definition~\ref{definition:complete-path} and \ref{definition:partial-path} that start in state $s$ by $\mathit{APaths}(s)$.  
...
We denote the set of potential paths that start in state $s$ by $\mathit{PPaths}(s)$.  We denote the set of all paths that start in state $s$ by $\mathit{Paths}(s)$.
...
We have that
\begin{align*}
\mathit{APaths}(-1) & = \{ -1, -1\ 0, -1\ 0\ 1, -1\ 0\ 2 \}\\
...
\[
\varphi \equiv \psi \mbox{ if } \satisfaction{\varphi}(s) = \satisfaction{\psi}(s) \mbox{ for all transition systems and } s \in S.
\]
\end{definition}
...
\end{array}
\right .\\
\textcolor{Green}{\mathit{Unsat}(\exists \nxt \varphi)} & \textcolor{Green}{= 
...
\end{align*}
\item
$\mathit{Sat}(\forall \always \varphi)$ is the largest $U \subseteq S$ satisfying
\[
U \subseteq 
\left \{
\begin{array}{ll}
S & \mbox{if $\varphi \equiv \TRUE$}\\
\{\, s \in F \mid s \in \mathit{Sat}(\varphi) \mbox{ and } \mathit{post}(s) \subseteq U \,\} & \mbox{otherwise}
\end{array}
\right .
\]
$\mathit{Unsat}(\forall \always \varphi)$ is the smallest $V \subseteq S$ satisfying
\[
V \supseteq 
...
\item 
\color{red}
Consider the CTL formula $\forall \nxt \varphi$.  For $\mathit{Sat}$, we distinguish two cases.
\begin{itemize}
\item 
Assume that $\varphi \equiv \TRUE$.  Then
\begin{align*}
\mathit{Sat}(\forall \nxt \varphi)
& = \{\, s \in S \mid \satisfaction{\forall \nxt \varphi}(s) = \top \,\}\\
& = \{\, s \in S \mid \satisfaction{\varphi}(\pi[1]) = \top \mbox{ for all } \pi \in \mathit{Paths}(s) \mbox{ with } |\pi| > 1 \,\}\\
& = \{\, s \in S \mid \satisfaction{\TRUE}(\pi[1]) = \top \mbox{ for all } \pi \in \mathit{Paths}(s) \mbox{ with } |\pi| > 1 \,\}
\comment{$\varphi \equiv \TRUE$}\\
& = S
\end{align*}
\item
Otherwise, $\varphi \not\equiv \TRUE$.  Then there exists $s_{\perp} \in \mathcal{S} \setminus S$ such that $\satisfaction{\varphi}(s_{\perp}) \not= \top$.  We distinguish two cases.
\begin{itemize}
\item 
Assume that $s \in S \setminus F$.  Then $s s_{\perp} \in \mathit{PPaths}(s)$.  Since $\satisfaction{\varphi}((s s_{\perp})[1]) = \satisfaction{\varphi}(s_{\perp}) \not= \top$, we can conclude that $\satisfaction{\varphi}(\pi[1]) \not= \top$ for some $\pi \in \mathit{Paths}(s)$ with $|\pi| > 1$.  
\item
...
= & \{\, s \in S \mid \satisfaction{\forall \nxt \varphi}(s) = \top \,\}\\
= & \{\, s \in S \mid \satisfaction{\varphi}(\pi[1]) = \top \mbox{ for all } \pi \in \mathit{Paths}(s) \mbox{ with } |\pi| > 1 \,\}\\
= & \{\, s \in S \setminus F \mid \satisfaction{\varphi}(\pi[1]) = \top \mbox{ for all } \pi \in \mathit{Paths}(s) \mbox{ with } |\pi| > 1 \,\} \cup\\
& \{\, s \in F \mid \satisfaction{\varphi}(\pi[1]) = \top \mbox{ for all } \pi \in \mathit{Paths}(s) \mbox{ with } |\pi| > 1 \,\}\\
= & \emptyset \cup
\{\, s \in F \mid \satisfaction{\varphi}(s') = \top \mbox{ for all } s' \in \mathit{post}(s) \,\}\\
= & \{\, s \in F \mid \mathit{post}(s) \subseteq \mathit{Sat}(\varphi) \,\}
\end{align*}
\end{itemize}
\color{Green}
...
\mathit{Unsat}(\forall \nxt \varphi)
= & \{\, s \in S \mid \satisfaction{\forall \nxt \varphi}(s) = \perp \,\}\\
= & \{\, s \in S \mid \satisfaction{\varphi}(\pi[1]) = \perp \mbox{ for some } \pi \in \mathit{APaths}(s) \mbox{ with } |\pi| > 1 \,\}\\
= & \{\, s \in S \mid \satisfaction{\varphi}(s') = \perp \mbox{ for some } s' \in \mathit{post}(s) \,\}\\
= & \{\, s \in S \mid \mathit{post}(s) \cap \mathit{Unsat}(\varphi) \not= \emptyset \,\}
\end{align*}
\color{blue}
\item
\textcolor{blue}{Who wants to try $\exists \nxt \varphi$?}
\color{Green}
\item
Consider the CTL formula $\forall \always \varphi$.  We first focus on $\mathit{Sat}$.

Given the CTL formula~$\varphi$, the function
\[
\mathcal{F}_{\varphi} : 2^S \to 2^S
\]
is defined by
\[
\mathcal{F}_{\varphi}(U) = \left \{
\begin{array}{ll}
S & \mbox{if $\varphi \equiv \TRUE$}\\
\{\, s \in F \mid s \in \mathit{Sat}(\varphi) \mbox{ and } \mathit{post}(s) \subseteq U \,\}  & \mbox{otherwise}
\end{array}
...
& = \{\, s \in S \mid \satisfaction{\forall \always \varphi}(s) = \top \,\}\\
& = \{\, s \in S \mid \satisfaction{\varphi}(\pi[i]) = \top \mbox{ for all } \pi \in \mathit{Paths}(s) \mbox{ and } 0 \leq i < |\pi| \,\}\\
& = \{\, s \in S \mid \satisfaction{\TRUE}(\pi[i]) = \top \mbox{ for all } \pi \in \mathit{Paths}(s) \mbox{ and } 0 \leq i < |\pi| \,\}
\comment{$\varphi \equiv \TRUE$}\\
& = S
\end{align*}
\item
...
Assume that $s \in S \setminus F$.  Then $s s_{\perp} \in \mathit{PPaths}(s)$.  Since $\satisfaction{\varphi}((s s_{\perp})[1]) = \satisfaction{\varphi}(s_{\perp}) \not= \top$, we can conclude that $\satisfaction{\varphi}(\pi[1]) \not= \top$ for some $\pi \in \mathit{Paths}(s)$ with $|\pi| > 1$.  
\item
Assume $s \in F$.  Then $\mathit{Paths(s)} = \{\, s \pi \mid s' \in \mathit{post}(s) \mbox{ and } \pi \in \mathit{Paths}(s') \,\}$.
\end{itemize}
From the above we can conclude that
\begin{align*}
\mathit{Sat}(\forall \always \varphi)
= & \{\, s \in S \mid \satisfaction{\forall \always \varphi}(s) = \top \,\}\\
= & \{\, s \in S \mid \satisfaction{\varphi}(\pi[i]) = \top \mbox{ for all } \pi \in \mathit{Paths}(s) \mbox{ and } 0 \leq i < |\pi| \,\}\\
= & \{\, s \in S \setminus F \mid \satisfaction{\varphi}(\pi[i]) = \top \mbox{ for all } \pi \in \mathit{Paths}(s) \mbox{ and } 0 \leq i < |\pi| \,\} \cup\\
= & \{\, s \in F \mid \satisfaction{\varphi}(\pi[i]) = \top \mbox{ for all } \pi \in \mathit{Paths}(s) \mbox{ and } 0 \leq i < |\pi| \,\}\\
= & \emptyset \cup \{\, s \in F \mid \satisfaction{\varphi}((s \pi)[i]) = \top  \mbox{ for all } s' \in \mathit{post}(s) \mbox{ and } \pi \in \mathit{Paths}(s') \mbox{ and } 0 \leq i < |\pi| \,\}\\
= & \{\, s \in F \mid \satisfaction{\varphi}(s) = \top \mbox{ and } \satisfaction{\varphi}(\pi[i]) = \top  \mbox{ for all } s' \in \mathit{post}(s) \mbox{ and } \pi \in \mathit{Paths}(s') \mbox{ and } 0 \leq i < |\pi| \,\}\\
= & \{\, s \in F \mid s \in \mathit{Sat}(\varphi) \mbox{ and } s' \in \mathit{Sat}(\forall \always \varphi) \mbox{ for all } s' \in \mathit{post}(s) \,\}\\
= & \{\, s \in F \mid s \in \mathit{Sat}(\varphi) \mbox{ and } \mathit{post}(s) \subseteq \mathit{Sat}(\forall \always \varphi) \,\}
\end{align*}
\end{itemize}

Let $U \subseteq S$ satisfying $U \subseteq \mathcal{F}_{\varphi}(U)$.  It remains to show that $U \subseteq \mathit{Sat}(\forall \always \varphi)$.  We distinguish two cases.
\begin{itemize}
\item 
If $\varphi \equiv \TRUE$ then $U \subseteq \mathcal{F}_{\varphi}(U) = S = \mathit{Sat}(\forall \always \varphi)$.
\item
Otherwise, $\varphi \not\equiv \TRUE$.  Towards a contradiction, assume that $s \in U$ and $s \not\in \mathit{Sat}(\forall \always \varphi)$.  Then $\satisfaction{\forall \always \varphi}(s) \not= \top$.  Hence, there exists $\pi \in \mathit{Paths}(s)$ and $0 \leq i < |\pi|$ such that $\satisfaction{\varphi}(\pi[i]) \not= \top$.  Next, we show that $\pi[j] \in U$ for all $0 \leq j \leq i$ by induction on $j$.  In the base case $j = 0$, we have that $\pi[0] = s \in U$.  Assume that $j > 0$.  By induction, $\pi[j-1] \in U$.  Since $U \subseteq \mathcal{F}_{\varphi}(U)$, we have that $\pi[j-1] \in F$ and $\mathit{post}(\pi[j-1]) \subseteq U$.  Hence, $\pi[j] \in U$.  Since $\pi[i] \in U$ and $U \subseteq \mathcal{F}_{\varphi}(U)$, we can conclude that $\pi[i] \in \mathit{Sat}(\varphi)$, which contradicts $\satisfaction{\varphi}(\pi[i]) \not= \top$.
\end{itemize}
...
Since $\mathit{APaths}(s) = \{ s \} \cup \{\, s \pi \mid s' \in \mathit{post}(s) \mbox{ and } \pi \in \mathit{APaths}(s') \,\}$, we have that
\begin{align*}
\mathit{Unsat}(\forall \always \varphi)
= & \{\, s \in S \mid \satisfaction{\forall \always \varphi}(s) = \perp \,\}\\
= & \{\, s \in S \mid \satisfaction{\varphi}(\pi[i]) = \perp \mbox{ for some } \pi \in \mathit{APaths}(s) \mbox{ and } 0 \leq i < |\pi| \,\}\\
= & \{\, s \in S \mid \satisfaction{\varphi}(s) = \perp \mbox{ or } \satisfaction{\varphi}(\pi[i]) = \perp \mbox{ for some } s' \in \mathit{post}(s) \mbox{ and } \pi \in \mathit{APaths}(s') \mbox{ and } 0 \leq i < |\pi| \,\}\\
...
\color{blue}
\item
Who wants to try $\exists \always \varphi$?
\end{itemize}
\end{proof}
