...
\end{align*}
\end{itemize}
For $\mathit{Unsat}$, we distinguish two cases.
\begin{itemize}
\item 
...
& = \{\, s \in S \mid \satisfaction{\FALSE}(\pi[1]) = \perp \mbox{ for some } \pi \in \mathit{APaths}(s) \mbox{ with } |\pi| > 1 \,\}\\
& \comment{$\varphi \equiv \FALSE$}\\
& = \{\, s \in S \mid \mbox{ there exists } \pi \in \mathit{APaths}(s) \mbox{ with } |\pi| > 1 \,\}\\
& = (S \setminus F) \cup \{\, s \in F \mid \mathit{post}(s) \not= \emptyset \,\}
\end{align*}
\item
Otherwise, 
