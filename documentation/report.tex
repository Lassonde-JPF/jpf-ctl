...
\end{tikzpicture}
\end{center}
If $\varphi \equiv \TRUE$ then $\exists \always \varphi \equiv \TRUE$ 
...
\begin{align*}
\satisfaction{\forall \nxt \varphi}(s) = \perp
\mbox{ iff } & \exists \pi \in \mathit{CoPaths}(s) \cup \mathit{PaPaths}(s) : |\pi| > 1 \wedge \satisfaction{\varphi}(\pi[1]) = \perp\\
...
\mbox{ iff } & \exists \pi \in \mathit{CoPaths}(s) : \forall 0 \leq i < |\pi| : \satisfaction{\varphi}(\pi[1]) = \perp\\
...
As a consequence, we can restrict our attention to CTL formulas defined by the grammar
\[
\varphi
::= a
\mid \neg \varphi
